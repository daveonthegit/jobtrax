...
statuses ||--o{ applications : "current_status (current_status_id)"
...
parsed_inputs ||--o{ application_status_history : "may_source (source_parse_id)"
```
...
- **Status → Applications (one-to-many):** Many applications share the same current status label (Applied, Interviewing, etc.) via `current_status_id`.
...
- **Parsed input → Status history (one-to-many, optional):** If a confirmed parse caused a status change, `source_parse_id` links the history row back to that parse for traceability. Manual status changes leave `source_parse_id` NULL.
...
| statuses | applications | 1 | 0..N | `applications.current_status_id` |
...
| parsed_inputs | application_status_history | 1 | 0..N | `application_status_history.source_parse_id` (nullable) |
